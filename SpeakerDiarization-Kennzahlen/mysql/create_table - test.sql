CREATE SCHEMA `diarisation-test` ;


CREATE TABLE IF NOT EXISTS `diarisation-test`.`session` (
...
CREATE TABLE IF NOT EXISTS `diarisation-test`.`speaker` (
...
CREATE TABLE IF NOT EXISTS `diarisation-test`.`speech` (
...
CREATE TABLE IF NOT EXISTS `diarisation-test`.`session_speaker` (
  ...
  CONSTRAINT `id_session`
    ...
    REFERENCES `diarisation-test`.`session` (`idSession`),
  CONSTRAINT `id_speaker`
    FOREIGN KEY (`id_speaker`)
    REFERENCES `diarisation-test`.`speaker` (`idspeaker`))
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4
COLLATE = utf8mb4_0900_ai_ci;

CREATE TABLE IF NOT EXISTS `diarisation-test`.`speaker_timeline` (
...
CREATE TABLE IF NOT EXISTS `diarisation-test`.`speaker_speech` (
  `id_speaker` INT NULL DEFAULT NULL,
  ...
  CONSTRAINT `id_speech`
    ...
    REFERENCES `diarisation-test`.`speech` (`idspeech`))
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4
COLLATE = utf8mb4_0900_ai_ci;