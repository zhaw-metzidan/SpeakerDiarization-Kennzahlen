CREATE TABLE IF NOT EXISTS `inselspiel2`.`session` (
...
CREATE TABLE IF NOT EXISTS `inselspiel2`.`speaker` (
...
CREATE TABLE IF NOT EXISTS `inselspiel2`.`session_speaker` (
  ...
  CONSTRAINT `id_session`
    ...
    REFERENCES `inselspiel2`.`session` (`idSession`),
  CONSTRAINT `id_speaker`
    FOREIGN KEY (`id_speaker`)
    REFERENCES `inselspiel2`.`speaker` (`idspeaker`))
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4
COLLATE = utf8mb4_0900_ai_ci;

CREATE TABLE IF NOT EXISTS `inselspiel2`.`speaker_timeline` (
...
CREATE TABLE IF NOT EXISTS `inselspiel2`.`speech` (
...
CREATE TABLE IF NOT EXISTS `inselspiel2`.`speaker_speech` (
  `id_speaker` INT NULL DEFAULT NULL,
  ...
  CONSTRAINT `id_speech`
    ...
    REFERENCES `inselspiel2`.`speech` (`idspeech`))
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4
COLLATE = utf8mb4_0900_ai_ci;