CREATE TABLE IF NOT EXISTS `inselspiel`.`session` (
...
CREATE TABLE IF NOT EXISTS `inselspiel`.`speaker` (
...
CREATE TABLE IF NOT EXISTS `inselspiel`.`session_speaker` (
  ...
  CONSTRAINT `id_session`
    ...
    REFERENCES `inselspiel`.`session` (`idSession`),
  CONSTRAINT `id_speaker`
    FOREIGN KEY (`id_speaker`)
    REFERENCES `inselspiel`.`speaker` (`idspeaker`))
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4
COLLATE = utf8mb4_0900_ai_ci;

CREATE TABLE IF NOT EXISTS `inselspiel`.`speaker_timeline` (
...
CREATE TABLE IF NOT EXISTS `inselspiel`.`speech` (
...
CREATE TABLE IF NOT EXISTS `inselspiel`.`speaker_speech` (
  `id_speaker` INT NULL DEFAULT NULL,
  ...
  CONSTRAINT `id_speech`
    ...
    REFERENCES `inselspiel`.`speech` (`idspeech`))
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4
COLLATE = utf8mb4_0900_ai_ci;